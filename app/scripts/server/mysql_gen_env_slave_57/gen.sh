...
mysql_install_dir=/usr/local/webserver/mysql-57
base_dir=$2
port=$1
file=$3
master_ip=$5
...
$mysql_install_dir/bin/mysqld  --defaults-file=$base_dir/$port/my.cnf  --initialize-insecure
...
bash ./secure_install.sh $base_dir/$port/mysql.sock $file $pos $master_ip $master_port $base_dir/$port
